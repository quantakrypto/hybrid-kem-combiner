...
Four files, and the difference between them is the most important thing in
this section.
...
| [`x25519-degenerate-v1.json`](vectors/x25519-degenerate-v1.json) | Adversarial: 10 cases, small-order `ek_T` and `ct_T` for the two Curve25519 suites | **No**, but the behaviour is. The expected values are this project's, and the rule they encode is X-Wing's: the all-zero `X25519` output is fed to the combiner, not rejected |

All four are language agnostic and run by **both** implementations.
